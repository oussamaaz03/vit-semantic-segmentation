...
def main(
    ...
    ema_model = ModelEMA(model, decay=0.9999, device=ptu.device)
    print(f"EMA model initialized with decay=0.9999")
    ...
    if resume and checkpoint_path.exists():
        ...
        if "ema_model" in checkpoint:
            ema_model.ema.load_state_dict(checkpoint["ema_model"])
            print(f"EMA model state restored from checkpoint")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if eval_epoch:
            # Evaluate using EMA model for better generalization
            ...
            print(f"Stats [{epoch}] (EMA):", eval_logger, flush=True)
            print("")
